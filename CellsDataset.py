...
class CellsDataset(utils.Dataset):
    CLASS_NAMES = ["cortex", "tubule_sain", "tubule_atrophique", "nsg_complet", "nsg_partiel", "pac", "vaisseau"]

    def __init__(self, train_ids, TRAIN_PATH):
        super().__init__()
    ...
    def load_cells(self, mode):
        ...
        if mode == "train":
            for n, id_ in enumerate(self.train_ids):
                if n < int(len(self.train_ids) * 0.9):
    ...
    def load_image(self, image_id):
        ...
        path = self.TRAIN_PATH + info
        img = imread(path + '/images/' + info + '.jp2')[:, :, :3]
    ...
    def load_mask(self, image_id):
        ...
        for masks_dir in os.listdir(path):
            temp_class_id = self.CLASS_NAMES.index(masks_dir) + 1
